...
def _get_api_key(name: str) -> str:
    """環境変数からAPIキーを取得する。Secrets登録時に紛れ込みうるBOM・空白・改行を除去する
    （BOM混入でGemini/Groqが全滅した実績があるための防御）"""
    return (os.environ.get(name) or "").replace("﻿", "").strip()
...
- opening_date: 開札日時。同上の形式。必ず文書内の「開札」の記載箇所から独立して抽出すること
  （bid_deadlineの値をそのまま転記しない）。入札締切と同じ日時が明記されている場合はその通りでよい。
- summary: 工事内容の概要を日本語2〜3文で。文書に書かれた事実のみを使う。
- 令和N年 は 2018+N 年に換算する（令和8年=2026年）。

出力形式（このJSONのみを出力）:
{"planned_price": ..., "region_requirement": ..., "koki": ..., "application_deadline": ..., "bid_deadline": ..., "opening_date": ..., "summary": ...}

=== 公告文書 ===
"""


# Gemini無料枠は「1日20リクエスト」しかない（429の quotaId: generate_content_free_tier_requests
# で実測確認）。上限に達したら同一実行内では以降Geminiを呼ばず、即Groqに切り替える。
...
def _call_gemini(prompt: str, pdf_data: bytes | None = None) -> str:
    ...
    if _gemini_daily_exhausted:
        raise ValueError("Gemini日次無料枠に到達済みのためスキップ")
    ...
    body = json.dumps({
        ...
        "generationConfig": {
            ...
            "temperature": 0.1,
            # 定型抽出に思考は不要。オフにすると応答が数十秒→2秒になり無料枠の浪費も防げる
    ...
    try:
        ...
        if e.code == 429 and "free_tier_requests" in detail:
            ...
            raise ValueError("Gemini日次無料枠(20回/日)に到達、以降はGroqを使用")
        raise
...
def _validate(result: dict) -> dict:
    ...
    return {
        ...
        "opening_date":         _clean_date(result.get("opening_date")),
...
def enrich_new_items(all_items: list, sent_ids: dict) -> None:
    ...
    for i, item in enumerate(capped):
        ...
        try:
            ...
            if not entry.get("opening_date") and fields["opening_date"]:
                entry["opening_date"] = fields["opening_date"]

            # 入札締切と開札日が同じ値の場合、それ自体は珍しくない（同日開札の案件は実在する）
            # ため自動修正はしない。ただしスクレイパー値とAI抽出値が食い違う場合は
            # スクレイパー側の取り違えの可能性があるため、確認用の注記だけ残す
            # （2026-07-17: 東京法務局の解体工事案件でスクレイパー側の開札日誤りが発覚した経緯あり）
            existing_bid     = entry.get("bid_deadline") or ""
            existing_opening = entry.get("opening_date") or ""
            if (existing_bid and existing_opening and existing_bid == existing_opening
                    and fields["opening_date"] and fields["opening_date"] != existing_opening):
                enrich["opening_date_note"] = (
                    "入札締切と開札日が同一値になっています。AI抽出では"
                    f"{fields['opening_date']} の可能性があるため、公告原本で開札日を確認してください。"
                )
